...
async function runPrintMode(prompt: string, options: any) {
  ...
  if (options.autoApprove) checker.setMode('auto');

  let initialMessages: MessageItem[] = [];
  if (options.continue) {
    const loaded = loadSession(DEFAULT_SESSION_ID);
    if (loaded) {
      initialMessages = loaded as MessageItem[];
    }
  }
  ...
  let hasOutput = false;
  const currentMessages: MessageItem[] = [...initialMessages];
...
async function main() {
  ...
  program
    ...
    .action(async (promptParts: string[], options) => {
      ...
      let currentMessages: MessageItem[] = [...initialMessages];
      ...
      render(
        <App
          ...
          initialInput={finalPrompt || undefined}
